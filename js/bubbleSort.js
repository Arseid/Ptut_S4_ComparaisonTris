...
function chartIt(array) {
    ...
    myChart = new Chart(ctx, {
        ...
        data: {
            labels: arrayIndex,
            datasets: [{
                label: "Graphique de l'algorithme Bubble-sort",
                data: data.ys,
                fill: true,
    ...
    for(let i = 0; i < getChoice.length; i++) {
        if(getChoice[i].type == "radio") {
            if(getChoice[i].checked) {
				// Vérifie quel radio button a été coché
...
function checkValues() {
    ...
    if ((sz.value < 5 || sz.value > 10000) && (s.value < 0 || s.value > 60)) {
        ...
        AfficherMasquer()
        getValueBubbleSort();
        bubbleSort(randomArrayValue,animationSpeed);
    }
}

/**
 * Gets the real results of Bubble sort
 * @author Siham Tabbaa
 * @returns {*}
 */
function getValueBubbleSort() {
    let i, j, tmp, counter = 0, array;
    let getRadio = radioButton();
    let size = sizes();
        ...
        for(j = 0; j < (size - i - 1 ); j++) {
            ...
            if(array[j] > array[j + 1]) {
                ...
                myChart.data.datasets.forEach((dataset) => {
                    ...
                    dataset.data[j]=array[j + 1];
                });;
                myChart.update();

                array[j+1] = tmp;
                myChart.data.datasets.forEach((dataset) => {
                    dataset.backgroundColor[j+1]="red";
                    dataset.data[j+1]=tmp;
                });;
                myChart.update();

                myChart.data.datasets.forEach((dataset) => {
                    dataset.backgroundColor[j]="yellow";
                    dataset.backgroundColor[j+1]="yellow";
                });;
                await waitform(500);
                myChart.update();
            }
