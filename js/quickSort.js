...
function chartIt(array) {
    ...
    myChart = new Chart(ctx, {
        ...
        data: {
            labels: arrayIndex,
            datasets: [{
                label: "Graphique de l'algorithme Insert-sort",
                data: data.ys,
                fill: true,
...
function checkValues() {
    ...
    if ((sz.value < 5 || sz.value > 10000) && (s.value < 0 || s.value > 60)) {
        ...
        AfficherMasquer();
        getValueQuickSort();
        doQuickSort(randomArrayValue,animationSpeed);
    }
}

let counterQuick = 0;
/**
 * Does Quick sort without animation parts
 * @author Yves-Robert Estrada
 * @param array
 * @param left
 * @param right
 * @returns {*}
 */
function trueDivision(array, left, right) {
 ...
 * @returns {*}
 */
function trueQuickSort(array, left, right) {
    let index;
	counterQuick++;
    if (array.length > 1) {
        index = trueDivision(array, left, right); //index retourné par la division
        if (left < index - 1) {
            trueQuickSort(array, left, index - 1);
        }
        if (index < right) {
            trueQuickSort(array, index, right);
        }
    }
    return "FIN : " + array;
}
...
/**
 * Gets the real results of Quick sort
 * @author Siham Tabbaa
 * @returns {*}
 */
function getValueQuickSort() {
    let array;
    let getRadio = radioButton();
    ...
    trueQuickSort(array,0,array.length-1);

    console.log("Tableau trié en : " + counterQuick + " tours");
	...
	document.getElementById("l1").innerHTML = counterQuick;
...
/**
 * Does Quick sort with animation parts
 * @author Yves-Robert Estrada
 * @param array
 * @param left
 * @param right
 ...
 * @returns {*}
 */
function quickSort(array, left, right,animationSpeed) {
    let index;
...
/**
 * Launches the Quick sort animation
 * @author Siham Tabbaa
 */
function doQuickSort(array,animationSpeed) {
