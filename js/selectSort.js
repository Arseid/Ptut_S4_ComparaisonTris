...
function checkValues() {
    ...
    if ((sz.value < 5 || sz.value > 10000) && (s.value < 0 || s.value > 60)) {
        ...
        AfficherMasquer();
        getValueSelectSort();
        selectSort(randomArrayValue,animationSpeed);
    }
}
...
/**
 * Does Select sort without animation parts
 * @author Yves-Robert Estrada
 * @returns {*}
 */
function trueSelectSort(array){
    let size = sizes();
...
/**
 * Gets the real results of Select sort
 * @author Siham Tabbaa
 * @returns {*}
 */
function getValueSelectSort() {
    let array;
    let getRadio = radioButton();
 ...
 * @returns {*}
 */
async function selectSort(array,animationSpeed){
    console.log(array);
    for (let i=0; i < array.length; i++){
        ...
        if (i !== min){
            ...
            myChart.data.datasets.forEach((dataset) => {
                dataset.backgroundColor[min]="green";
                dataset.data[min]=array[i];
            });;
            myChart.update();
            ...
            myChart.data.datasets.forEach((dataset) => {
                ...
                dataset.backgroundColor[min]="yellow";
            });;
            await waitform(500);
            myChart.update();
        }
