...
function checkValues() {
    ...
    if ((sz.value < 5 || sz.value > 10000) && (s.value < 0 || s.value > 60)) {
        ...
        AfficherMasquer()
        getValueInsertSort();
        insertSort(randomArrayValue,animationSpeed);
    }
}
...
/**
 * Does Insert sort without animation parts
 * @author Siham Tabbaa
 * @param array The array to sort
 * @returns {*}
 */
function trueInsertSort(array){
	let i,j,current;
    ...
    for (i = 1; i < array.length; i++) {
        ...
        counter++;  //à chaque itération, cmpt augmente de i
        console.log("Itération numéro : " + counter + "<br>" + array + "<br>"); // affichage du tableau et du numéro de l'itération actuelle
    }
    return array;
}

/**
 * Gets the real results of Insert sort
 * @author Siham Tabbaa
 * @returns {*}
 */
function getValueInsertSort() {
    let array;
    let getRadio = radioButton();
...
/**
 * Animates Insert sort
 * @author Yves-Robert Estrada
 * @param array The array to sort
 * @param animationSpeed The speed of the animation
 * @returns {*}
 */
async function insertSort(array,animationSpeed) {
    for (let i = 1; i < array.length; i++) {
        ...
        await waitform(500);
        myChart.data.datasets.forEach((dataset) => {
            dataset.data[j + 1]=current;
            dataset.backgroundColor[0]="green";
